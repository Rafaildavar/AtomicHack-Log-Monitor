"""ML анализатор аномалий для Telegram бота с использованием sentence-transformers."""
...
class MLAnomalyAnalyzer:
    """ML анализатор аномалий с использованием трансформеров."""

    def __init__(self, similarity_threshold: float = 0.7):
        """Инициализация ML анализатора.

        Args:
            similarity_threshold: Порог уверенности для сопоставления аномалий (0.0-1.0)
        """
    ...
    def find_anomalies_with_ml(self, logs_df: pd.DataFrame, anomalies_dict: pd.DataFrame) -> pd.DataFrame:
        """Находит аномалии в логах с использованием ML.

        Args:
            logs_df: DataFrame с логами (только WARNING и ERROR)
            anomalies_dict: DataFrame со словарем аномалий

        Returns:
            DataFrame с найденными аномалиями и проблемами
        """
        logger.info(f"Начинаю ML анализ: {len(logs_df)} строк логов, {len(anomalies_dict)} аномалий")
        ...
        known_anomalies = anomalies_dict["Аномалия"].astype(str).tolist()
        ...
        for _, row in logs_df.iterrows():
            if row["level"] == "WARNING":
                ...
                related_problems = anomalies_dict[
                    anomalies_dict["Аномалия"] == matched_text
                ]
                ...
                for _, ap in related_problems.iterrows():
                    ...
                    # Ищем ERROR строки, где текст содержит проблему
